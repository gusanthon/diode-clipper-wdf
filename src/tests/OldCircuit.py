...
class Circuit:
    def __init__(self, fs, elements, source, root, output) -> None:
        self.fs = fs
    ...
    def record_mono_audio(
        ...
        p.terminate()
        print(
            f"AVG time between reading from in buffer to writing to out buffer :\n {round(np.average(times),5)} ms\n"
        )
...
class DiodeClipper(Circuit):
    def __init__(
        ...
        super().__init__(elements, sample_rate, self.Vs, self.Dp, self.C1)
